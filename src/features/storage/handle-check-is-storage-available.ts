...
const handleCheckIsStorageAvailable = async ({ parentFolderData }: HandleCheckIsStorageAvailable): Promise<boolean> => {
  ...
  try {
    ...
    const isSubMoveSameRootFolder = parentFolderData && parentFolderData.root_folder_user_id === createUserId;
    if (isSubMoveSameRootFolder) return true;
